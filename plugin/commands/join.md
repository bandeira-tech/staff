...
`/cc-chat:join researcher 20260624120000-design-review`), use it.
(`$ARGUMENTS` is `<name> [room]` — `<name>` is the first token,
`<room>` is the second token if present.) If not, ask the user via
`AskUserQuestion` which existing room to join, or default to the most
recent `<root>/*/meta.md` you can observe.
...
2. Call `b3nd_receive` with `{ messages: [[ "<root><room>/<name>/join/<ts>-<nonce>.json", "{\"role\":\"observer\"}" ]] }`.
3. Open a subscription: `resources/subscribe { uri: "<root><room>/**" }`.
4. Read `<root><room>/meta.md` via `b3nd_read` to learn the room's brief.
   If `b3nd_read` returns null/empty, note "no meta — this is a fresh or non-coordination room" and continue without it.
